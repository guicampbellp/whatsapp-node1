...
app.post('/processar-imagem', async (req, res) => {
  try {
    console.log('Recebida requisição para processar imagem');
    ...
    const tempImagePath = path.join(tempDir, `temp_${Date.now()}.${arquivoUrl.split('.').pop().split('?')[0]}`);
    console.log(`Arquivo temporário: ${tempImagePath}`);

    try {
      console.log(`Iniciando download da imagem: ${arquivoUrl}`);
      const response = await axios({
        method: 'get',
        url: arquivoUrl,
        responseType: 'stream'
      ...
      console.log('Imagem baixada com sucesso, iniciando processamento...');
      exec(`node extrair-img.js "${tempImagePath}"`, async (error, stdout, stderr) => {
        console.log('Processamento concluído, stdout:', stdout);
        console.log('stderr:', stderr);
        ...
        if (error) {
          console.error('Erro ao processar imagem:', stderr);
          return res.status(500).json({ error: 'Falha ao processar imagem', details: stderr });
        }
      ...
      return res.status(500).json({ 
        error: 'Falha ao baixar imagem da URL fornecida',
        details: downloadError.message 
      });
    }
  } catch (err) {
